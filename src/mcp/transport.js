 ...
 *
 * IMPORTANT: stdout is the protocol channel — never write logs there (use
 * stderr). Streams are injectable so the transport is testable with PassThrough.
 *
 * Returns a promise that resolves once input ends AND all in-flight responses
 * have been written (so tests can deterministically await completion).
 */
function runStdioServer({ server, input = process.stdin, output = process.stdout } = {}) {
  ...
  function dispatchLine(line) {
    ...
    const p = Promise.resolve()
      ...
      .catch((e) => {
        ...
        write({ jsonrpc: '2.0', id, error: { code: -32603, message: `Internal error: ${e && e.message ? e.message : e}` } });
      });
    pending.push(p);
  }
  ...
  return new Promise((resolve) => {
    ...
    const finish = async () => {
      if (buf.length) { dispatchLine(buf); buf = ''; }
